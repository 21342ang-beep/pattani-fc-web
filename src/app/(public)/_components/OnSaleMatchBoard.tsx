...
export default function OnSaleMatchBoard({ match, zoneQS = "" }: { match: OnSaleMatch; zoneQS?: string }) {
  return <article className="overflow-hidden rounded-2xl bg-gradient-to-br from-green-950 via-green-900 to-emerald-800 text-white shadow-xl">
    <div className="grid gap-6 p-6 sm:p-8 md:grid-cols-[1fr_auto] md:items-center md:p-10">
      ...
      <div className="border-t border-white/20 pt-5 md:min-w-60 md:border-l md:border-t-0 md:pl-8 md:pt-0"><p className="flex items-center gap-2 text-sm text-emerald-100"><Calendar className="size-4 text-yellow-300" />{match.kickoffAt ? formatDateTime(match.kickoffAt) : "ยังไม่กำหนดวันแข่ง"}</p><p className="mt-2 flex items-center gap-2 text-sm text-emerald-100"><MapPin className="size-4 text-yellow-300" />{match.venue ?? "ยังไม่กำหนดสนาม"}</p><p className="mt-5 text-sm text-emerald-100">ราคา</p><p className="text-2xl font-black text-yellow-300">แยกตามโซน</p><Link href={zoneQS ? `/matches/${match.id}${zoneQS}` : "/tickets"} className="mt-5 block rounded-lg bg-yellow-300 px-5 py-3 text-center text-sm font-bold text-green-950 transition hover:bg-yellow-200">เลือกโซนและจอง</Link></div>
    </div>
  </article>;
}
