...
import { ArrowLeft, Barcode, CalendarDays, MapPin, Ticket } from "lucide-react";
    ...
    <div className="mx-auto max-w-3xl px-4 py-8 md:py-12">
      ...
      <article className="overflow-hidden rounded-3xl bg-green-950 text-white shadow-2xl">
        <header className="bg-gradient-to-r from-green-800 to-green-700 px-6 py-6 md:px-9">
          ...
          <div className="mt-3 flex items-end justify-between gap-3"><div><h1 className="text-3xl font-black md:text-4xl">{tier?.name ?? pass.tierId}</h1><p className="mt-1 text-green-100">ฤดูกาล {pass.seasonLabel}</p></div><Ticket className="size-10 text-yellow-300" /></div>
        </header>
        <div className="grid gap-6 p-6 md:grid-cols-[1fr_250px] md:p-9">
          <div><p className="text-sm text-green-200">ผู้ถือบัตร</p><p className="mt-1 text-2xl font-black">{pass.customerName}</p>
            <div className="mt-6 space-y-3 border-y border-white/15 py-5 text-sm"><p className="flex items-center gap-2"><MapPin className="size-4 text-yellow-300" /> โซนที่นั่ง: <strong>{pass.seatZone}</strong></p><p className="flex items-center gap-2"><CalendarDays className="size-4 text-yellow-300" /> สมัครเมื่อ: {formatDateTime(pass.createdAt)}</p><p className="font-mono text-xs text-green-200">Pass no. {pass.passCode}</p></div>
