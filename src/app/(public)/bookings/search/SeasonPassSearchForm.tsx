      ...
      <form action={formAction} className="mt-6 rounded-2xl border border-green-100 bg-white p-6 shadow-sm">
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="block text-sm font-semibold text-green-900">
            ชื่อผู้สมัคร
            <input name="customerName" minLength={2} maxLength={100} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal outline-none focus:border-green-600 focus:ring-2 focus:ring-green-100" placeholder="ชื่อที่ใช้สมัครบัตรรายปี" />
          </label>
          <label className="block text-sm font-semibold text-green-900">
            เบอร์โทรศัพท์
            <input name="customerPhone" inputMode="tel" maxLength={20} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal outline-none focus:border-green-600 focus:ring-2 focus:ring-green-100" placeholder="เบอร์ที่ใช้สมัคร" />
          </label>
        </div>
        <p className="mt-3 text-xs text-slate-500">กรอกชื่อหรือเบอร์โทรศัพท์อย่างน้อยหนึ่งช่อง</p>
        ...
        <button disabled={pending} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-green-800 px-5 py-3 text-sm font-bold text-yellow-300 transition hover:bg-green-900 disabled:opacity-60">
          <Search className="size-4" /> {pending ? "กำลังค้นหา..." : "ค้นหาบัตรรายปี"}
        </button>
      </form>
