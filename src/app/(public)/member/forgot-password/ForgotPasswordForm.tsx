...
export default function ForgotPasswordForm() {
  ...
  if (requestState?.requested) return <form action={resetAction} className="mt-6 space-y-4"><p className="rounded-lg bg-green-50 p-3 text-sm text-green-800">หากเบอร์นี้เป็นบัญชีที่ยืนยันแล้ว ระบบจะส่ง OTP กรุณาตรวจสอบข้อความและกรอกรหัสด้านล่าง</p><input name="pin" inputMode="numeric" autoComplete="one-time-code" required placeholder="รหัส OTP" className="w-full rounded-lg border border-slate-300 px-4 py-3 text-center tracking-[0.4em]" /><PasswordInput name="password" autoComplete="new-password" required placeholder="รหัสผ่านใหม่" /><PasswordInput name="confirmPassword" autoComplete="new-password" required placeholder="ยืนยันรหัสผ่านใหม่" />{resetState?.error && <p className="text-sm text-red-700">{resetState.error}</p>}<button disabled={resetting} className="w-full rounded-lg bg-green-800 py-3 font-bold text-yellow-300">{resetting ? "กำลังบันทึก..." : "ตั้งรหัสผ่านใหม่"}</button></form>;
